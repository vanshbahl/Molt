...
| **Old-pass → new-fail reproducibility** | **Probed and confirmed** (2026-09-05, PyJWT 1.7.1 → 2.10.1): old call succeeds; identical call raises `DecodeError` on 2.x. See [feasibility_probes.json](../experiments/feasibility_probes.json). Note: this specific probe exercises the `algorithms=` requirement, which the registered bounded pilot task (below, and [protocol.md §3](../experiments/protocol.md#3-pilot-migration-selection--resolved-candidates-and-bounded-scope-only)) excludes. A second, separately-recorded probe (`pyjwt-1-to-2-bounded-task` in the same file) reproduces old-pass→new-fail for the two sites actually in scope — the `jwt.ExpiredSignature` exception rename and the `verify_expiration=` flat-kwarg restructuring — and additionally found the restructuring site fails without an exception at the call site (the scripted 2026-09-23 re-run in [experiments/probes/](../experiments/probes/) shows 2.10.1 emits only a generic "unsupported kwargs" DeprecationWarning) rather than with a clean error, which is the concrete basis for that site's "not yet fixture-tested" caveat above. |
