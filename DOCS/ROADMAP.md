...
**Progress (2026-09-05):** Literature review — done pre-existing. Eight candidate scorecards — **done**, see [CANDIDATES.md](CANDIDATES.md), including three real old-pass/new-fail reproductions (not just documentation). Pilot selection — **done** (PyJWT/SQLAlchemy/Pydantic, each with an explicit bounded task scope). A first, dated protocol slice — **done**, see [experiments/protocol.md](../experiments/protocol.md). **Same-day follow-up:** model/provider selection and the PyJWT pilot's frozen development exemplar are now **resolved** (Anthropic `claude-haiku-4-5-20251001`; exemplar backed by a fresh real probe of the exact bounded-task sites), with a ready-to-execute evidence packet/config and a real-price-schedule cost projection — see [experiments/protocol.md §§10–13](../experiments/protocol.md#10-model-provider-selection-for-the-pyjwt-pilot--resolved), [experiments/pilot_config.json](../experiments/pilot_config.json), [experiments/pilot_estimate.json](../experiments/pilot_estimate.json), [experiments/ceilings.json](../experiments/ceilings.json). **Still open, by design:** pilot repository identities/SHAs (Phase 6 has not started), the V1 repair cap, and — the single blocker keeping M0 from closing — an actual **measured** (not projected) token/latency/retry/cost pilot result, which requires a billable API key and spend authorization this pass did not have. Projected worst-case pilot cost is under 5 US cents. M0 is therefore **not closed**.
...
| --- | --- |
| M0 — Feasibility and methodology registered | Closest work/RuleFlow review; 6–8 scorecards; pilot feasibility/cost; RQ1–RQ4; exemplar/caching/repair policies; numeric expansion/stop/split/audit rules and protocol hash. **Partial (2026-09-05):** scorecards ([CANDIDATES.md](CANDIDATES.md)), pilot selection, model/provider selection, PyJWT exemplar, and estimate-derived ceilings ([experiments/protocol.md](../experiments/protocol.md) §§1–13, [ceilings.json](../experiments/ceilings.json)) done. **Blocked on one named item:** a real billed pilot call — measured token/latency/retry/cost data does not exist, only a real-price-schedule projection ([pilot_estimate.json](../experiments/pilot_estimate.json)); the V1 repair cap and final expansion/split numbers depend on it. |
