...
**M0 progress so far:** eight candidates were investigated with primary-source evidence and, for three of them, a real isolated-venv old-pass/new-fail reproduction — see [DOCS/CANDIDATES.md](DOCS/CANDIDATES.md). A recommended three-migration pilot (PyJWT anchor, SQLAlchemy Tier 2, Pydantic ambiguity/DSL-ceiling) is proposed there and registered, with its bounded task scope, in [experiments/protocol.md](experiments/protocol.md). A follow-up pass (also 2026-09-05) closed the PyJWT pilot's model/provider selection (Anthropic `claude-haiku-4-5-20251001`) and its frozen development exemplar, backed by a fresh, real, zero-cost probe of the two exact bounded-task sites, and produced a ready-to-execute evidence packet, pinned config, and a price-schedule-based (not billed) cost projection — see [experiments/protocol.md §§10–13](experiments/protocol.md#10-model-provider-selection-for-the-pyjwt-pilot--resolved).

**M0 is still not closed**, for one specific, named reason: **no real billed LLM call has been made.** Everything closable with development-only, zero-or-near-zero-cost evidence has been closed (model/provider, the PyJWT exemplar, an evidence-packet-size-and-real-pricing cost projection, and estimate-derived or policy-default numeric ceilings — see [experiments/ceilings.json](experiments/ceilings.json)). What remains genuinely blocked is measuring *actual* token usage, latency, retries, and monetary cost for the PyJWT pilot, which needs a billable API key and explicit spend authorization neither of which this pass had (projected worst-case cost is under 5 US cents — see [experiments/pilot_estimate.json](experiments/pilot_estimate.json)). The V1 repair cap and the SQLAlchemy/Pydantic pilots' own model configs and exemplars remain open for the same reason plus their own unstarted scope. The final migration/repository counts remain open pending Phase 6 curation, which has still not started.

**Next milestone: close M0** — execute the configured PyJWT pilot call(s) once a billable key and spend authorization exist, record the measured results in place of the current projections, and select the V1 repair cap from that evidence. Follow the [roadmap](DOCS/ROADMAP.md); do not jump to engine implementation. Dynamic Python behavior, partial oracles, small samples, and contamination limit conclusions. Recency does not prove absence from model training. V1 excludes product dashboards, hosted services, automatic production PRs, and broader language or analysis platforms.
